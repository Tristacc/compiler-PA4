...
public class SymbolTableVisitor implements Visitor {
  ...
  public Object visit(ClassDecl node, Object data){ 
    ...
    node.i.accept(this, data+"$" + i.s);
    //visit the vardecllist node
    if(node.v!=null){
      node.v.accept(this, data + "$" + i.s);
    }
    //visit the methoddecllist node
    if(node.m!=null){
      node.m.accept(this, data + "$" + i.s);
    }
    //add the class to the symbol table wit with required prefix
    symbolTable.classes.put(data+"$"+i.s, node);
    symbolTable.typeName.put(data+"$"+i.s,"*class");
